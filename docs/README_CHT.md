...
**基於 AI 大模型的 A�?港股/美股 智能分析系統**

自動分析自選�?�?生成決策儀表盤 �?多渠道推送（Telegram/Discord/郵件/企業微信/飛書�?
**零成本部�?* · GitHub Actions 免費運行 · 無需伺服�?
[**功能特�?*](#-功能特�? · [**快速開�?*](#-快速開�? · [**推送效�?*](#-推送效�? · [**完整指南**](full-guide.md) · [**常見問題**](FAQ.md) · [**更新日誌**](CHANGELOG.md)
...
| 分析 | 多維度分�?| 技術面 + 籌碼分布 + 輿情情報 + 實時行情 |
| 市場 | 全球市場 | 支援 A股、港股、美�?|
| 復盤 | 大盤復盤 | 每日市場概覽、板塊漲跌、北向資�?|
| 回測 | AI 回測驗證 | 自動評估歷史分析準確率，方向勝率、止盈止損命中率 |
| 推�?| 多渠道通知 | Telegram、Discord、郵件、企業微信、飛書等 |
| 自動�?| 定時運行 | GitHub Actions 定時執行，無需伺服�?|
...
點擊右上�?`Fork` 按鈕（順便點�?Star 支持一下）

#### 2. 配置 Secrets

進入�?Fork 的倉庫 �?`Settings` �?`Secrets and variables` �?`Actions` �?`New repository secret`
...
| `OPENAI_API_KEY` | OpenAI 兼容 API Key（支�?DeepSeek、通義千問等） | 可選 |
...
| `TELEGRAM_MESSAGE_THREAD_ID` | Telegram Topic ID (用於發送到子話�? | 可選 |
| `DISCORD_WEBHOOK_URL` | Discord Webhook URL | 可選 |
| `DISCORD_BOT_TOKEN` | Discord Bot Token（與 Webhook 二選一�?| 可選 |
| `DISCORD_CHANNEL_ID` | Discord Channel ID（使�?Bot 時需要） | 可選 |
| `EMAIL_SENDER` | 發件人郵箱（�?`xxx@qq.com`�?| 可選 |
...
| `CUSTOM_WEBHOOK_URLS` | 自定�?Webhook（支持釘釘等，多個用逗號分隔�?| 可選 |
| `CUSTOM_WEBHOOK_BEARER_TOKEN` | 自定�?Webhook �?Bearer Token（用於需要認證的 Webhook�?| 可選 |
| `SINGLE_STOCK_NOTIFY` | 單股推送模式：設為 `true` 則每分析完一隻股票立即推�?| 可選 |
| `REPORT_TYPE` | 報告類型：`simple`(精簡) �?`full`(完整)，Docker環境推薦設為 `full` | 可選 |
| `ANALYSIS_DELAY` | 個股分析和大盤分析之間的延遲（秒），避免API限流，如 `10` | 可選 |

> 至少配置一個渠道，配置多個則同時推送。更多配置請參�?[完整指南](full-guide.md)
...
| Secret 名稱 | 說明 | 必填 |
|------------|------|:----:|
| `STOCK_LIST` | 自選股代碼，�?`600519,hk00700,AAPL,TSLA` | �?|
...
> 📖 本地運行、Docker 部署詳細步驟請參�?[完整配置指南](full-guide.md)
...
🟢 買入 | 貴州茅台(600519)
📌 縮量回踩MA5支撐，乖離率1.2%處於最佳買�?💰 狙擊: 買入1800 | 止損1750 | 目標1900
✅多頭排�?✅乖離安�?✅量能配�?
🟡 觀�?| 寧德時代(300750)
📌 乖離�?.8%超過5%警戒線，嚴禁追高
⚠️ 等待回調至MA5附近再考慮
...
> 📖 完整環境變量、定時任務配置請參�?[完整配置指南](full-guide.md)

## 🧩 FastAPI Web 服務（可選）

本地運行時，可啟�?FastAPI 服務來管理配置和觸發分析�?
### 啟動方式

| 命令 | 說明 |
|------|------|
| `python main.py --serve` | 啟動 API 服務 + 執行一次完整分�?|
| `python main.py --serve-only` | 僅啟�?API 服務，手動觸發分�?|

- 訪問地址：`http://127.0.0.1:8000`
- API 文檔：`http://127.0.0.1:8000/docs`

### 功能特�?
- 📝 **配置管理** - 查看/修改自選股列�?- 🚀 **快速分�?* - 通過 API 接口觸發分析
- 📊 **實時進度** - 分析任務狀態實時更新，支持多任務並�?- 📈 **回測驗證** - 評估歷史分析準確率，查詢方向勝率與模擬收�?
### API 接口
...
| `/api/v1/analysis/status/{task_id}` | GET | 查詢任務狀�?|
...
├── main.py              # 主程序入�?├── server.py            # FastAPI 服務入口
├── src/                 # 核心業務代碼
�?  ├── analyzer.py      # AI 分析器（Gemini�?�?  ├── config.py        # 配置管理
�?  ├── notification.py  # 消息推�?�?  ├── storage.py       # 數據存儲
�?  └── ...
├── api/                 # FastAPI API 模塊
├── bot/                 # 機器人模�?├── data_provider/       # 數據源適配器
├── docker/              # Docker 配置
�?  ├── Dockerfile
�?  └── docker-compose.yml
├── docs/                # 項目文檔
...
> 📢 以下功能將視後續情況逐步完成，如果你有好的想法或建議，歡�?[提交 Issue](https://github.com/zhaowl1993/daily_stock_analysis/issues) 討論�?
### 🔔 通知渠道擴展
- [x] 企業微信機器�?- [x] 飛書機器�?- [x] Telegram Bot
- [x] 郵件通知（SMTP�?- [x] 自定�?Webhook（支持釘釘、Discord、Slack、Bark 等）
- [x] iOS/Android 推送（Pushover�?- [x] 釘釘機器�?（已支持命令交互 >> [相關配置](bot/dingding-bot-config.md)�?### 🤖 AI 模型支持
- [x] Google Gemini（主力，免費額度�?- [x] OpenAI 兼容 API（支�?GPT-4/DeepSeek/通義千問/Claude/文心一言 等）
- [x] 本地模型（Ollama�?
### 📊 數據源擴�?- [x] AkShare（免費）
- [x] Tushare Pro
- [x] Baostock
...
歡迎提交 Issue �?Pull Request�?
詳見 [貢獻指南](CONTRIBUTING.md)

## License
[MIT License](../LICENSE) © 2026 ZhuLinsen

如果你在項目中使用或基於本項目進行二次開發�?非常歡迎�?README 或文檔中註明來源並附上本倉庫鏈接�?這將有助於項目的持續維護和社區發展�?
## 聯繫與合�?- GitHub Issues：[提交 Issue](https://github.com/zhaowl1993/daily_stock_analysis/issues)
